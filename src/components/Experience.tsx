    ...
    <section id="experiencia" className="bg-background-alt py-24">
      <div className="mx-auto max-w-6xl px-6 text-center lg:px-10">
        ...
        <div className="relative mx-auto mt-16 max-w-4xl text-left">
          <span className="absolute left-[19px] top-2 bottom-2 w-px bg-white/10" />
...
function CompanyBranch({ company, delay }: { company: CompanyTimeline; delay: number }) {
  return (
    <div className="flex gap-5">
      <span
        className="z-10 mt-1 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full text-[12px] font-bold text-[#06121c] ring-4 ring-background-alt"
      ...
      <div className="flex-1">
        <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
          <h3 className="font-display text-lg font-semibold">{company.company}</h3>
          <span className="text-sm text-muted">{company.overallPeriod}</span>
        </div>
...
function RoleCard({ role, color }: { role: RoleStep; color: string }) {
  return (
    <div className="w-72 max-w-full rounded-[16px] border border-border-subtle bg-surface p-5 transition-colors hover:border-white/20">
      <span
